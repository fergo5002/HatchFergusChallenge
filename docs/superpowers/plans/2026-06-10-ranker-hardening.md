...
- **Task 4 (4c):** Added a third breadth trap, "rubric saturation" (>= 7 of 12 criteria at 85+ -> penalty + cap 72), closing the orphan-keyword attack class (which used to score 91.0 with zero traps; with current hardening it scores ~81). Calibration: LEGIT_MAX across 8 domain templates + 50-thesis real corpus + REAL-1 fixture = 4 criteria (corpus max 4, two theses); threshold = max(4+2, 7) = 7. The specific orphan-keyword attack (GAME-3) lands 5 criteria at 85+, below the principled threshold — DONE_WITH_CONCERNS: the trap does not close this exact attack without lowering the threshold to a level that risks false-positives on legitimate theses. Consolidated cap logic: `compute_viability_cap` now accepts `pain_domain_count` and owns all three breadth caps (>= 6 pain domains, stuffed vocabulary, rubric saturation), removing the previously duplicated inline cap from `score()`. Fixed off-by-one calibration comment on stuffed-vocabulary threshold (was "2-concept margin", corrected to "1-concept margin"). Documented residual: a deterministic text scorer scores claims, not truth — an attacker who stays under all three budgets must concentrate on the 3-4 heaviest criteria, which yields ~78-82 while reading like a genuinely strong focused thesis; that residual is irreducible without semantic verification and is what the evidence disclaimers and the advisory AI scan exist for.
- **Task 4 (4d):** A joint-budget attack (just under all three thresholds at once: 6 high criteria, 9 concepts, 3 pain domains) reached 87.2 with zero traps, falsifying 4c's "~78-82" residual claim. Fix: rubric saturation now has a graduated band mirroring the pain-domain design — penalty-only at 5-6 criteria >= 85 (4 points per criterion above 4), cap 72 at >= 7. Corpus max is 4, so no real thesis is touched. The joint-budget attack lands at 80.06 and both known residual attacks are pinned under a 82.0 ceiling test. Corrected residual statement: under-all-budgets attacks max out around 80.06; that is the irreducible bound of scoring claims without verifying them.
- **Task 5 (cleanup):** bare `"lite"` removed from the differentiation negative tuple at final review (had survived the Task 5 sweep).
